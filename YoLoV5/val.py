...
Usage:
    $ python path/to/val.py --data coco128.yaml --pt yolov5s.pt --img 640
"""
...
from YoLoV5.general import coco80_to_coco91_class, check_dataset, check_file, check_img_size, check_requirements, \
    box_iou, non_max_suppression, scale_coords, xyxy2xywh, xywh2xyxy, set_logging, increment_path, colorstr
from YoLoV5.metrics import ap_per_class, ConfusionMatrix
from YoLoV5.plots import plot_images, output_to_target, plot_study_txt
from YoLoV5.torch_utils import select_device, time_sync
...
def run(data,
    ...
    if save_json and len(jdict):
        w = Path(weights[0] if isinstance(weights, list) else weights).stem if weights is not None else ''  # pt
...
def main(opt):
    ...
    if opt.task in ('train', 'val', 'test'):  # run normally
        ...
        # python val.py --task study --data coco.yaml --iou 0.7 --pt yolov5s.pt yolov5m.pt yolov5l.pt yolov5x.pt
